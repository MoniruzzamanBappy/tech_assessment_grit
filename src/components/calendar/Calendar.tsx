...
async function getData(dates: { startDate: string; endDate: string }): Promise<{
  ...
  try {
    const result = await httpRequest.get(
      `https://api.bytebeds.com/api/v1/property/1/room/rate-calendar/assessment?start_date=${dates?.startDate}&end_date=${dates?.endDate}`
    );

    if (result?.code === "Succeed") {
      const rows: IRootObject = processData(result?.data as APIRootObject[]);
      const datesBetween: DatesBetween = getDatesBetween(
        dates?.startDate,
        dates?.endDate
      );
      const onlyDates: string[] = getOnlyDatesBetween(
        dates?.startDate,
        dates?.endDate
      );
...
const Calendar = () => {
  ...
  const [datesBetween, setDatesBetween] = useState<DatesBetween>(
    {} as DatesBetween
  );
  ...
  return (
    <Stack spacing={2} m={3}>
      <Box component={Paper} p={1} sx={{ border: "2px solid grey" }}>
        <Typography py={1} variant={"h4"}>Rate Calendar</Typography>
        <Box width={"20%"}>
            ...
            >
              <TableHead>
                <TableRow>
                  ...
                  {datesBetween &&
                    Object.keys(datesBetween).map((monthYear) => (
                      <TableCell
                        key={monthYear}
                        colSpan={datesBetween[monthYear].length}
                      >
                        <b>{monthYear}</b>
                      </TableCell>
                    ))}
                ...
                <TableRow>
                  ...
                  {datesBetween &&
                    Object.keys(datesBetween).map((monthYear) =>
                      datesBetween[monthYear].map(
                        (dateInfo: ICalendarFormat) => (
                    ...
                    <React.Fragment key={roomName}>
                      <TableRow>
                        <TableCell
                          colSpan={
                            datesBetween &&
                            Object.keys(datesBetween).reduce(
                              (sum, key) => sum + datesBetween[key].length,
                              0
                            )
                          }
                        >
                      ...
                      <TableRow>
                        ...
                        {onlyDates?.map((date: string) => {
                          const item = rows[roomName]?.inventory?.find(
                            (inv: any) => inv.date === date
                          );
                          return (
                            <TableCell
                              sx={
                                item?.status
                                  ? { bgcolor: "#008000", color: "#fff" }
                      ...
                      {rows[roomName]?.rate_plans &&
                        Object.keys(rows[roomName]?.rate_plans).map(
                          (ratePlanName, index) => (
                            <React.Fragment key={ratePlanName}>
                              <TableRow>
                                <TableCell>
                                  <Typography>{ratePlanName}</Typography>
                                  <Stack
                                    spacing={2}
                                    direction={"row"}
                                    alignItems={"center"}
                                ...
                                {rows[roomName]?.rate_plans[ratePlanName]?.map(
                                  (ratePlan: IRates, index: number) => (
